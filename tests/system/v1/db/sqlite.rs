...
#[sqlx::test]
async fn session_with_empty_username(pool: SqlitePool) {
    let db = SqliteDatabase::from_pool(pool).await.unwrap();

    let id = Uuid::new_v4();
    let result = db.create_session(id, SprocketCommand::Run, "").await;

    assert!(matches!(
        result,
        Err(sprocket::system::v1::db::DatabaseError::Validation(msg)) if msg == "`created_by` cannot be empty for an session"
    ));
}
...
async fn run_with_empty_name(pool: SqlitePool) {
    ...
    assert!(matches!(
        result,
        Err(sprocket::system::v1::db::DatabaseError::Validation(msg)) if msg == "`name` cannot be empty for a run"
    ));
}
...
async fn run_with_empty_source(pool: SqlitePool) {
    ...
    assert!(matches!(
        result,
        Err(sprocket::system::v1::db::DatabaseError::Validation(msg)) if msg == "`source` cannot be empty for a run"
    ));
}
...
async fn run_with_empty_directory(pool: SqlitePool) {
    ...
    let run_id = Uuid::new_v4();
    let result = db
        .create_run(run_id, session_id, "test", "/test.wdl", "{}", "")
        .await;

    assert!(matches!(
        result,
        Err(sprocket::system::v1::db::DatabaseError::Validation(msg)) if msg == "`directory` cannot be empty for a run"
    ));
}
