...
impl TestContext {
    ...
    async fn add_analysis_rule(&self, rule: Box<dyn AnalysisRule>) -> anyhow::Result<()> {
        ...
        let validator = Arc::new(Validator::empty);
...
async fn verify_examples(ctx: Arc<TestContext>, expected_rule: &str) -> Result<(), Failed> {
    ...
    for result in results {
        ...
        let (_, file_name) = document_path
            .rsplit_once(['/', '\\'])
            .expect("should be an absolute path");
        let is_negative_snippet = file_name.contains("negative-");

        let example_index = file_name
        ...
        let parse_diagnostics = result.document().parse_diagnostics();
