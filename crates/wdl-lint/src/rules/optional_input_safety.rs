...
/// Returns `true` if `expr` is already safe for optional interpolation: either
/// it is not optional, or it uses `if`/`else`, `defined` / `select_first` /
/// `select_all`, or string concatenation where each optional operand is handled
/// the same way.
fn optional_expr_is_guarded<C: EvaluationContext>(
    ...
    match expr {
        Expr::If(_) => true,
        Expr::Call(call) => guard_call(call),
        Expr::Addition(a) => {
            let (left, right) = a.operands();
            optional_expr_is_guarded(evaluator, &left)
                && optional_expr_is_guarded(evaluator, &right)
        }
        Expr::Parenthesized(p) => optional_expr_is_guarded(evaluator, &p.expr()),
        _ => false,
    }
}
...
impl Visitor for OptionalInputSafetyRule {
    ...
    fn command_section(
        ...
        for part in section.parts() {
            ...
            if optional_expr_is_guarded(&mut evaluator, &expr) {
                continue;
            }
