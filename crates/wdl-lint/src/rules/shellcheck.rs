...
fn to_bash_var(placeholder: &Placeholder, ty: Option<Type>) -> (String, bool, Option<Span>) {
    ...
    let expr_span = placeholder.expr().span();

    if let Some(Type::Primitive(pty, _)) = ty {
        match pty {
            PrimitiveType::Integer | PrimitiveType::Float => {
                return ("4".repeat(placeholder_len), true, Some(expr_span));
            }
            ...
            PrimitiveType::String => {
                if evaluates_to_bash_literal(&placeholder.expr()) {
                    return ("a".repeat(placeholder_len), true, Some(expr_span));
                }
            }
            _ => {}
        }
