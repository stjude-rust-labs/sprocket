...
pub(crate) fn container(
    ...
    let fallback = default.unwrap_or(DEFAULT_TASK_REQUIREMENT_CONTAINER);

    let Some((_, value)) = entry else {
        // SAFETY: `FromStr` for `ContainerSource` is infallible.
        return vec![fallback.parse().unwrap()];
    };
    ...
    if let Some(array) = value.as_array() {
        array
            ...
            .map(|v| {
                ...
                let s = s.as_ref();
                // SAFETY: `FromStr` for `ContainerSource` is infallible.
                if s == WILDCARD_CONTAINER { fallback } else { s }
                    .parse()
                    .unwrap()
            })
            .collect()
...
mod tests {
    ...
    fn container_returns_default_when_unset() {
        let result = container(&TaskInputs::default(), &HashMap::new(), None);
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0],
            ContainerSource::Docker(DEFAULT_TASK_REQUIREMENT_CONTAINER.to_string())
        );
    }

    #[test]
    fn container_returns_custom_default_when_unset() {
        let result = container(&TaskInputs::default(), &HashMap::new(), Some("alpine:3.18"));
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0],
    ...
    fn container_returns_single_image() {
        let requirements =
            map_with_value(TASK_REQUIREMENT_CONTAINER, PrimitiveValue::new_string("foo:bar").into());
        let result = container(&TaskInputs::default(), &requirements, None);
        assert_eq!(result.len(), 1);
    ...
    fn container_resolves_single_wildcard_to_default() {
        let requirements =
            map_with_value(TASK_REQUIREMENT_CONTAINER, PrimitiveValue::new_string("*").into());
        let result = container(&TaskInputs::default(), &requirements, None);
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0],
    ...
    fn container_resolves_single_wildcard_to_custom_default() {
        let requirements =
            map_with_value(TASK_REQUIREMENT_CONTAINER, PrimitiveValue::new_string("*").into());
        let result = container(&TaskInputs::default(), &requirements, Some("debian:12"));
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0],
            ContainerSource::Docker("debian:12".to_string())
        );
    }
    ...
    fn container_resolves_wildcard_in_array_with_custom_default() {
        ...
        let result = container(&TaskInputs::default(), &requirements, Some("alpine:3.18"));
    ...
    fn respect_inputs_over_requirements() {
        ...
        assert_eq!(
            container(&inputs, &requirements, None)
                .first()
                .unwrap()
                .to_string(),
            "foo:bar"
