...
workflow literal_array_inlining {
    ...
    Array[String] barely_over = ["alpha", "bravo", "charlie", "delta", "india", "november"]

    # The starting column counts too, so the array that fits above does not here.
    if (true) {
        Array[String] deeper_copy = ["alpha", "bravo", "charlie", "delta", "echo", "november"]
    }

    # So does whatever follows the closing bracket, comments included.
    Array[Int] with_short_comment = [
        1,
        2,
        3,
    ]  # fits
    Array[Int] with_long_comment = [1, 2, 3]  # this comment leaves the array no room at all

    # A nested array is measured with the delimiters and separators around it.
    Array[Int] nested = flatten([
        [
            1,
            2,
            3,
