...
task multiple_placeholders_per_line {
    command <<<
        --clip3pAdapterMMp ~{clip_3p_adapter_mmp.left} ~{(if (length(read_twos) != 0) then clip_3p_adapter_mmp.right
        else None)} \
...
task idempotent {
    command <<<
        ...
        STAR --readFilesIn \
            ...
            --alignSJstitchMismatchNmax ~{sep(" ", quote([align_sj_stitch_mismatch_n_max.noncanonical_motifs,
                 align_sj_stitch_mismatch_n_max.GT_AG_and_CT_AC_motif, align_sj_stitch_mismatch_n_max.GC_AG_and_CT_GC_motif,
