...
workflow seaseq {
    ...
    Array[File] actual_bowtie_index = select_first([bowtie_idx_2.bowtie_indexes, bowtie_idx.bowtie_indexes,
         bowtie_index])
    ...
    if (!defined(spikein_bowtie_index) && defined(spikein_reference)) {
        ...
        call bowtie.index as spikein_bowtie_idx { input: reference = select_first([spikein_reference,
             string_spikein]) }
    }
    ...
    if (defined(spikein_bowtie_index)) {
        ...
        if (length(int_spikein_bowtie_index) != 6) {
            # create bowtie index if 6 index files aren't provided
            call bowtie.index as spikein_bowtie_idx_2 { input: reference = select_first([spikein_reference,
                 string_spikein]) }
        }
    }
    Array[File] actual_spikein_bowtie_index = select_first([spikein_bowtie_idx_2.bowtie_indexes,
         spikein_bowtie_idx.bowtie_indexes, spikein_bowtie_index, string_spikein_buffer])
    ...
    if (defined(spikein_bowtie_index) || defined(spikein_reference)) {
        scatter (eachfastq in original_fastqfiles) {
            call fastqc.fastqc as spikein_indv_fastqc { input: inputfile = eachfastq, default_location
                = if (one_fastq) then sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/SpikeIn/FastQC"
            else "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/SpikeIn/FastQC" 
            }
            call util.basicfastqstats as spikein_indv_bfs { input: fastqfile = eachfastq, default_location
            ...
            call bowtie.spikein_SE as spikein_indv_map { input: fastqfile = eachfastq, index_files
                = actual_spikein_bowtie_index, metricsfile = spikein_indv_bfs.metrics_out, default_location
                = if (one_fastq) then sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/SpikeIn/SummaryStats"
            else "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/SpikeIn/SummaryStats" 
            }
        }
    ...
    if (multi_fastq) {
        scatter (eachfastq in fastqfiles) {
            ...
            call fastqc.fastqc as indv_fastqc { input: inputfile = eachfastq, default_location
                = "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/QC/FastQC" 
            }

            call util.basicfastqstats as indv_bfs { input: fastqfile = eachfastq, default_location
                = "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/QC/SummaryStats" 
            }

            call mapping.mapping as indv_mapping { input: fastqfile = eachfastq, index_files
                = actual_bowtie_index, metricsfile = indv_bfs.metrics_out, blacklist = blacklist,
                 default_location = "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz",
                "") + "/BAM_files" }

            call fastqc.fastqc as indv_bamfqc { input: inputfile = indv_mapping.sorted_bam,
                 default_location = "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz",
                "") + "/QC/FastQC" }

            call runspp.runspp as indv_runspp { input: bamfile = select_first([indv_mapping.bklist_bam,
                 indv_mapping.sorted_bam]) }

            call bedtools.bamtobed as indv_bamtobed { input: bamfile = select_first([indv_mapping.bklist_bam,
                 indv_mapping.sorted_bam]) }

            call util.evalstats as indv_summarystats { input: fastq_type = "SEAseq Sample FASTQ",
                 bambed = indv_bamtobed.bedfile, sppfile = indv_runspp.spp_out, fastqczip
                = indv_fastqc.zipfile, bamflag = indv_mapping.bam_stats, rmdupflag = indv_mapping.mkdup_stats,
                 bkflag = indv_mapping.bklist_stats, fastqmetrics = indv_bfs.metrics_out, default_location
                = "SAMPLE/" + sub(basename(eachfastq), ".fastq.gz|.fq.gz", "") + "/QC/SummaryStats" 
            }
        ...
        call util.mergehtml { input: htmlfiles = indv_summarystats.xhtml, txtfiles = indv_summarystats.textfile,
             default_location = "SAMPLE", outputfile = "AllMapped_" + length(fastqfiles) + "_seaseq-summary-stats.html" 
        }

        call samtools.mergebam { input: bamfiles = indv_mapping.sorted_bam, metricsfiles = indv_bfs.metrics_out,
             default_location = if defined(results_name) then results_name + "/BAM_files"
        else "AllMerge_" + length(indv_mapping.sorted_bam) + "_mapped" + "/BAM_files", outputfile
            = if defined(results_name) then results_name + ".sorted.bam" else "AllMerge_"
            + length(fastqfiles) + "_mapped.sorted.bam" }

        call fastqc.fastqc as mergebamfqc { input: inputfile = mergebam.mergebam, default_location
            = sub(basename(mergebam.mergebam), ".sorted.b.*$", "") + "/QC/FastQC" }

        call samtools.indexstats as mergeindexstats { input: bamfile = mergebam.mergebam, default_location
            = sub(basename(mergebam.mergebam), ".sorted.b.*$", "") + "/BAM_files" }
        ...
        if (defined(blacklist)) {
            ...
            call bedtools.intersect as merge_rmblklist { input: fileA = mergebam.mergebam, fileB
                = blacklist_file, default_location = sub(basename(mergebam.mergebam), ".sorted.b.*$",
                "") + "/BAM_files", nooverlap = true }
            call samtools.indexstats as merge_bklist { input: bamfile = merge_rmblklist.intersect_out,
        ...
        call samtools.markdup as merge_markdup { input: bamfile = mergebam_afterbklist, default_location
            = sub(basename(mergebam_afterbklist), ".sorted.b.*$", "") + "/BAM_files" }

        call samtools.indexstats as merge_mkdup { input: bamfile = merge_markdup.mkdupbam, default_location
            = sub(basename(mergebam_afterbklist), ".sorted.b.*$", "") + "/BAM_files" }
    ...
    if (one_fastq) {
        ...
        call fastqc.fastqc as uno_fastqc { input: inputfile = fastqfiles[0], default_location
            = sub(basename(fastqfiles[0]), ".fastq.gz|.fq.gz", "") + "/QC/FastQC" }

        call util.basicfastqstats as uno_bfs { input: fastqfile = fastqfiles[0], default_location
            = sub(basename(fastqfiles[0]), ".fastq.gz|.fq.gz", "") + "/QC/SummaryStats" }

        call mapping.mapping { input: fastqfile = fastqfiles[0], index_files = actual_bowtie_index,
             metricsfile = uno_bfs.metrics_out, blacklist = blacklist, default_location = sub(
            basename(fastqfiles[0]
        ), ".fastq.gz|.fq.gz", "") + "/BAM_files" }

        call fastqc.fastqc as uno_bamfqc { input: inputfile = mapping.sorted_bam, default_location
            = sub(basename(fastqfiles[0]), ".fastq.gz|.fq.gz", "") + "/QC/FastQC" }

        call runspp.runspp as uno_runspp { input: bamfile = select_first([mapping.bklist_bam,
             mapping.sorted_bam]) }

        call bedtools.bamtobed as uno_bamtobed { input: bamfile = select_first([mapping.bklist_bam,
             mapping.sorted_bam]) }
    ...
    call macs.macs { input: bamfile = sample_bam, pvalue = "1e-9", keep_dup = "auto", egs
        = egs.genomesize, default_location = sub(basename(sample_bam), ".sorted.b.*$", "")
        + "/PEAKS/NARROW_peaks" + "/" + basename(sample_bam, ".bam") + "-p9_kd-auto", coverage_location
        = sub(basename(sample_bam), ".sorted.b.*$", "") + "/COVERAGE_files/NARROW_peaks" + "/"
        + basename(sample_bam, ".bam") + "_p9_kd-auto" }

    call util.addreadme { input: default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/PEAKS" }

    call macs.macs as all { input: bamfile = sample_bam, pvalue = "1e-9", keep_dup = "all",
         egs = egs.genomesize, default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/PEAKS/NARROW_peaks" + "/" + basename(sample_bam, ".bam") + "-p9_kd-all", coverage_location
        = sub(basename(sample_bam), ".sorted.b.*$", "") + "/COVERAGE_files/NARROW_peaks" + "/"
    ...
    call macs.macs as nomodel { input: bamfile = sample_bam, nomodel = true, egs = egs.genomesize,
        ...
        + "/" + basename(sample_bam, ".bam") + "-nm", coverage_location = sub(basename(sample_bam
    ), ".sorted.b.*$", "") + "/COVERAGE_files/NARROW_peaks" + "/" + basename(sample_bam, ".bam"
    ) + "_nm" }

    call bamtogff.bamtogff { input: gtffile = gtf, chromsizes = samtools_faidx.chromsizes, bamfile
        = select_first([merge_markdup.mkdupbam, mapping.mkdup_bam]), bamindex = select_first(
    [merge_mkdup.indexbam, mapping.mkdup_index]
    ), default_location = sub(basename(sample_bam), ".sorted.b.*$", "") + "/BAM_Density" }

    call bedtools.bamtobed as forsicerbed { input: bamfile = select_first([merge_markdup.mkdupbam,
         mapping.mkdup_bam]) }

    call sicer.sicer { input: bedfile = forsicerbed.bedfile, chromsizes = samtools_faidx.chromsizes,
         genome_fraction = egs.genomefraction, fragmentlength = select_first([uno_bfs.readlength,
         mergebam.avg_readlength]), default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/PEAKS/BROAD_peaks", coverage_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/COVERAGE_files/BROAD_peaks" }

    call rose.rose { input: gtffile = gtf, bamfile = select_first([merge_markdup.mkdupbam, mapping.mkdup_bam
    ]), bamindex = select_first([merge_mkdup.indexbam, mapping.mkdup_index]), bedfile_auto
        = macs.peakbedfile, bedfile_all = all.peakbedfile, default_location = sub(basename(
        sample_bam
    ), ".sorted.b.*$", "") + "/PEAKS/STITCHED_peaks" }

    call runspp.runspp { input: bamfile = sample_bam }

    call util.peaksanno { input: gtffile = gtf, bedfile = macs.peakbedfile, chromsizes = samtools_faidx.chromsizes,
         summitfile = macs.summitsfile, default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/PEAKS_Annotation/NARROW_peaks" + "/" + sub(basename(macs.peakbedfile), "_peaks.bed",
        "") }

    call util.peaksanno as all_peaksanno { input: gtffile = gtf, bedfile = all.peakbedfile,
         chromsizes = samtools_faidx.chromsizes, summitfile = all.summitsfile, default_location
        = sub(basename(sample_bam), ".sorted.b.*$", "") + "/PEAKS_Annotation/NARROW_peaks"
        + "/" + sub(basename(all.peakbedfile), "_peaks.bed", "") }

    call util.peaksanno as nomodel_peaksanno { input: gtffile = gtf, bedfile = nomodel.peakbedfile,
         chromsizes = samtools_faidx.chromsizes, summitfile = nomodel.summitsfile, default_location
        = sub(basename(sample_bam), ".sorted.b.*$", "") + "/PEAKS_Annotation/NARROW_peaks"
        + "/" + sub(basename(nomodel.peakbedfile), "_peaks.bed", "") }

    call util.peaksanno as sicer_peaksanno { input: gtffile = gtf, bedfile = sicer.scoreisland,
         chromsizes = samtools_faidx.chromsizes, default_location = sub(basename(sample_bam
    ), ".sorted.b.*$", "") + "/PEAKS_Annotation/BROAD_peaks" }

    # Motif Analysis
    if (run_motifs) {
        call motifs.motifs { input: reference = reference, reference_index = samtools_faidx.faidx_file,
             bedfile = macs.peakbedfile, motif_databases = motif_databases, default_location
            = sub(basename(sample_bam), ".sorted.b.*$", "") + "/MOTIFS" }

        call util.flankbed { input: bedfile = macs.summitsfile, default_location = sub(basename(
            sample_bam
        ), ".sorted.b.*$", "") + "/MOTIFS" }

        call motifs.motifs as flank { input: reference = reference, reference_index = samtools_faidx.faidx_file,
             bedfile = flankbed.flankbedfile, motif_databases = motif_databases, default_location
            = sub(basename(sample_bam), ".sorted.b.*$", "") + "/MOTIFS" }
    }

    call viz.visualization { input: wigfile = macs.wigfile, chromsizes = samtools_faidx.chromsizes,
         xlsfile = macs.peakxlsfile, default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/COVERAGE_files/NARROW_peaks" + "/" + sub(basename(macs.peakbedfile), "_peaks.bed",
        "") }

    call viz.visualization as vizall { input: wigfile = all.wigfile, chromsizes = samtools_faidx.chromsizes,
         xlsfile = all.peakxlsfile, default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/COVERAGE_files/NARROW_peaks" + "/" + sub(basename(all.peakbedfile), "_peaks.bed",
        "") }

    call viz.visualization as viznomodel { input: wigfile = nomodel.wigfile, chromsizes = samtools_faidx.chromsizes,
         xlsfile = nomodel.peakxlsfile, default_location = sub(basename(sample_bam), ".sorted.b.*$",
        "") + "/COVERAGE_files/NARROW_peaks" + "/" + sub(basename(nomodel.peakbedfile), "_peaks.bed",
        "") }

    call viz.visualization as vizsicer { input: wigfile = sicer.wigfile, chromsizes = samtools_faidx.chromsizes,
    ...
    call bedtools.intersect { input: fileA = macs.peakbedfile, fileB = sortbed.sortbed_out,
         countoverlap = true, sorted = true }
    ...
    if (one_fastq) {
        call util.evalstats as uno_summarystats {
            ...
            input: fastq_type = "SEAseq Sample FASTQ", bambed = finalbed.bedfile, sppfile
            = runspp.spp_out, fastqczip = select_first([uno_bamfqc.zipfile, string_qual]), bamflag
            = mapping.bam_stats, rmdupflag = mapping.mkdup_stats, bkflag = mapping.bklist_stats,
             fastqmetrics = uno_bfs.metrics_out, countsfile = intersect.intersect_out, peaksxls
            = macs.peakxlsfile, enhancers = rose.enhancers, superenhancers = rose.super_enhancers,
        ...
        call util.summaryreport as uno_overallsummary {
            ...
            input: overallqc_html = uno_summarystats.xhtml, overallqc_txt = uno_summarystats.textfile 
        }
    }  # end if one_fastq
    ...
    if (multi_fastq) {
        call util.evalstats as merge_summarystats {
            ...
            input: fastq_type = "SEAseq Comprehensive", bambed = finalbed.bedfile, sppfile
            = runspp.spp_out, fastqczip = select_first([mergebamfqc.zipfile, string_qual]),
             bamflag = mergeindexstats.flagstats, rmdupflag = merge_mkdup.flagstats, bkflag
            = merge_bklist.flagstats, countsfile = intersect.intersect_out, peaksxls = macs.peakxlsfile,
             enhancers = rose.enhancers, superenhancers = rose.super_enhancers, default_location
            = sub(basename(sample_bam), ".sorted.b.*$", "") + "/QC/SummaryStats" }

        call util.summaryreport as merge_overallsummary {
            # Presenting all quality stats for the analysis
            input: sampleqc_html = mergehtml.xhtml, overallqc_html = merge_summarystats.xhtml,
             sampleqc_txt = mergehtml.mergetxt, overallqc_txt = merge_summarystats.textfile 
        }
    }  # end if multi_fastq
