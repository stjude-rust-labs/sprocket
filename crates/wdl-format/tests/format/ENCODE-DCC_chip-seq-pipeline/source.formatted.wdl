...
workflow chip {
    ...
    File ref_fa_ = select_first([
        ref_fa,
        read_genome_tsv.ref_fa,
    ])
    File? bwa_idx_tar_ = (
        if defined(bwa_idx_tar)
        then bwa_idx_tar
    ...
    File chrsz_ = select_first([
        chrsz,
        read_genome_tsv.chrsz,
    ])
    String gensz_ = select_first([
        gensz,
    ...
    Boolean enable_xcor_ = (
        if pipeline_type == "control"
        then false
        else true
    )
    Boolean enable_count_signal_track_ = (
        if pipeline_type == "control"
        then false
    ...
    Int num_rep_nodup_bam = (
        if length(nodup_bams) < num_rep_bam
        then num_rep_bam
        else length(nodup_bams)
    )
    Int num_rep_ta = (
        if length(tas) < num_rep_nodup_bam
        then num_rep_nodup_bam
        else length(tas)
    )
    Int num_rep_peak = (
        if length(peaks) < num_rep_ta
        then num_rep_ta
        else length(peaks)
    )
    ...
    Int num_ctl_nodup_bam = (
        if length(ctl_nodup_bams) < num_ctl_bam
        then num_ctl_bam
        else length(ctl_nodup_bams)
    )
    Int num_ctl_ta = (
        if length(ctl_tas) < num_ctl_nodup_bam
