...
impl DiagnosticsConfig {
    ...
    fn severity_mut(&mut self, id: &str) -> Option<&mut Option<Severity>> {
        match id {
            ...
            ExceptDirectiveValidRule::ID => Some(&mut self.except_directive_valid),
            CommandSectionIndentationRule::ID => Some(&mut self.command_section_indentation),
            DeprecatedObjectRule::ID => Some(&mut self.deprecated_object),
            DeprecatedPlaceholderRule::ID => Some(&mut self.deprecated_placeholder),
            DeprecatedRuntimeSectionRule::ID => Some(&mut self.deprecated_runtime_section),
            _ => None,
        }
    }
