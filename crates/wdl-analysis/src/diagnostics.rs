...
pub fn recursive_enum(name: &str, span: Span, ty: &str) -> Diagnostic {
    // Unlike `recursive_struct`, which labels individual members, an `enum` has a single
    // type for all of its choices. Just highlight the `enum` name, as its type as a *whole* is
    // recursive.
