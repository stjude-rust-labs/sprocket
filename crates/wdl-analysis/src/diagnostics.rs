...
pub fn non_literal_enum_value(span: Span) -> Diagnostic {
    Diagnostic::error("enum variant value must be a literal expression")
        .with_highlight(span)
        .with_fix(
            "enum values must be literal expressions only (string literals, \
             numeric literals, collection literals, or struct literals); \
             string interpolation, variable references, and computed expressions \
             are not allowed",
        )
}
