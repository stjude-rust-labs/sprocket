#!/bin/sh
# Derived from https://github.com/rust-lang/rustup/blob/main/rustup-init.sh
...
BINARY_NAME="sprocketup"
LATEST_RELEASE_ROOT="https://github.com/Serial-ATA/sprocket/releases/latest/download" # TODO: update this before merge
# Set quiet as a global for ease of use
QUIET=no

usage() {
    cat <<EOF
${BINARY_NAME}-init 0.1.0

The installer for ${BINARY_NAME}

Usage: ${BINARY_NAME}-init[EXE] [OPTIONS]

Options:
  -v, --verbose
          Set log level to 'DEBUG' if 'RUST_LOG' is unset
  -q, --quiet
          Disable progress output, set log level to 'WARN' if 'RUST_LOG' is unset
  -y
...
main() {
    ...
    determine_architecture || return 1
    local _arch="$RETVAL"
    ...
    local _url="${LATEST_RELEASE_ROOT}/${BINARY_NAME}-${_arch}${_ext}"
    ...
    local _file="${_dir}/${BINARY_NAME}${_ext}"
    ...
    if [ ! -x "$_file" ]; then
        ...
        err "Please copy the file to a location where you can execute binaries and run ./${BINARY_NAME}${_ext}."
        exit 1
    fi
...
determine_architecture() {
  ...
  if [ "$_ostype" = Linux ]; then
      if ldd --version 2>&1 | grep -q 'musl'; then
        err "musl libc is unsupported"
        exit 1
      fi
  fi

  _bitness=$(getconf LONG_BIT)
  if [[ "$_bitness" -ne 64 ]]; then
      err "${BINARY_NAME} only supports 64 bit"
      exit 1
  fi
    ...
    if check_cmd curl; then
        ...
        if echo "$_curl_path" | grep "/snap/" > /dev/null 2>&1; then
            if check_cmd wget; then
                ...
                err "curl installed with snap cannot be used to install Sprocket"
...
get_ciphersuites_for_curl() {
    if [ -n "${SPROCKETUP_TLS_CIPHERSUITES-}" ]; then
        # user specified custom cipher suites, assume they know what they're doing
        RETVAL="$SPROCKETUP_TLS_CIPHERSUITES"
        return
    fi
...
set +u
case "$SPROCKETUP_INIT_SH_PRINT" in
    arch | architecture)
        determine_architecture || exit 1
        echo "$RETVAL"
        ;;
    *)
        main "$@" || exit 1
