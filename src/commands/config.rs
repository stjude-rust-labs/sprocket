//! Implementation of the config command.

use anyhow::Context;
use clap::Parser;
use clap::Subcommand;

use crate::commands::CommandError;
use crate::commands::CommandResult;
use crate::config::Config;

/// The [Taplo schema directive] for `sprocket.toml`.
///
/// [Taplo schema directive]: https://taplo.tamasfe.dev/configuration/directives.html#the-schema-directive
const SCHEMA_DIRECTIVE: &str = "#:schema https://raw.githubusercontent.com/stjude-rust-labs/sprocket/refs/heads/main/jsonschemas/sprocket.toml.json";

/// Arguments for the `config` subcommand.
#[derive(Parser, Debug, Clone)]
pub struct Args {
    /// Subcommand for the `config` command.
    #[command(subcommand)]
    command: ConfigSubcommand,
}

impl Args {
    /// Returns `true` if the subcommand is 'Default'.
    pub fn is_default(&self) -> bool {
        matches!(self.command, ConfigSubcommand::Default)
    }
}

/// Subcommands for the `config` command.
#[derive(Subcommand, Debug, Clone)]
pub enum ConfigSubcommand {
    /// Print the JSON schema for `sprocket.toml` to stdout.
    Schema,

    /// Generates a default configuration file.
    Default,

    /// Displays the current configuration.
    Resolve(ResolveArgs),
}

/// Arguments for the `config resolve` subcommand.
#[derive(Parser, Debug, Clone)]
pub struct ResolveArgs {
    /// Unredacts any redacted secrets in the configuration.
    #[clap(long)]
    unredact: bool,
}

/// Header prepended to `sprocket config default` output.
const DEFAULT_HEADER: &str = "\
# Sprocket configuration file.
#
# This file was generated by `sprocket config default` and contains the default
# values. Sprocket does not require a configuration file; save any settings you
# want to change as `sprocket.toml`.
#
# Lint and validation rules are configured under `[check.rules.<RuleId>]`. Each
# rule may set `severity` (one of `off`, `note`, `warning`, or `error`) and any
# parameters that apply to it. For example:
#
#     [check.rules.NamingConvention]
#     severity = \"error\"
#     allowed_names = [\"GATK\"]
#
# Run `sprocket explain <RuleId>` to see a rule's configurable parameters, or
# `sprocket explain --list-all-rules` to list every rule.
";

/// Runs the `config` command.
pub fn config(args: Args, mut config: Config) -> CommandResult<()> {
    let (config, header, include_schema_directive) = match args.command {
        ConfigSubcommand::Schema => {
            let schema = schemars::schema_for!(Config);
            let schema_pretty =
                serde_json::to_string_pretty(&schema).context("serializing config schema")?;
            println!("{schema_pretty}");
            return Ok(());
        }
        ConfigSubcommand::Default => (Config::default(), Some(DEFAULT_HEADER), true),
        ConfigSubcommand::Resolve(args) => {
            // Redact any secrets unless explicitly requested not to
            if !args.unredact {
                config.run.engine = config.run.engine.redact();
            }

            (config, None, false)
        }
    };

    let serialized = toml_spanner::to_string(&config)
        .context("failed to serialize configuration")
        .map_err(CommandError::Single)?;

    let mut output = String::new();
    if include_schema_directive {
        output.push_str(SCHEMA_DIRECTIVE);
        output.push_str("\n\n");
    }
    match header {
        Some(header) => output.push_str(&format!("{header}\n{serialized}")),
        None => output.push_str(&serialized),
    }
    println!("{output}");
    Ok(())
}
