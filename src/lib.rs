...
pub use config::NotificationEvent;
pub use config::NotificationsConfig;
pub use config::ServerConfig;
pub use config::WebhookConfig;
pub use config::WebhookKind;
...
cfg_select! {
    all(debug_assertions, windows) => {
        /// Calls the provided callback on a thread with a larger stack.
        ///
        /// A thread is created with a 2 MiB stack size to parse the options due
        /// to outstanding `clap_derive` issue https://github.com/clap-rs/clap/issues/5134.
        ///
        /// This works around hitting a stack overflow due to the very large
        /// stack frames generated by the derive macro.
        #[inline]
        fn with_large_stack<F, T>(cb: F) -> T
        where
            F: Fn() -> T + Send + 'static,
            T: Send + 'static,
        {
    ...
    _ => {
        /// Calls the provided callback directly.
        #[inline]
        fn with_large_stack<F, T>(cb: F) -> T
        where
            F: Fn() -> T + Send + 'static,
